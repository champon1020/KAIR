...
def main(json_path="options/vrt/001_train_vrt_videosr_bi_reds_6frames.json"):
    ...
    for epoch in range(1000000):  # keep running
        for i, train_data in enumerate(train_loader):
            ...
            if opt["use_static_graph"] and (
                current_step == opt["train"]["fix_iter"] - 1
            ):
                current_step += 1
                model.update_learning_rate(current_step)
                model.save(current_step)
                ...
                for idx, test_data in enumerate(test_loader):
                    ...
                    for i in range(output.shape[0]):
                        ...
                        if opt["val"]["save_img"]:
                            ...
                            util.mkdir(save_dir)
                            seq_ = os.path.basename(test_data["lq_path"][i][0]).split(
                                "."
                            )[0]
                            os.makedirs(f"{save_dir}/{folder[0]}", exist_ok=True)
                            cv2.imwrite(
                                f"{save_dir}/{folder[0]}/{seq_}_{current_step:d}.png",
                                img,
                            )
                        ...
                        if img_gt.ndim == 3:  # RGB image
                            ...
                            img_gt = (
                                util.bgr2ycbcr(img_gt.astype(np.float32) / 255.0)
                                * 255.0
                            )
                    ...
                    if gt is not None:
                        ...
                        logger.info(
                            "Testing {:20s}  ({:2d}/{})".format(
                                folder[0], idx, len(test_loader)
                            )
                        )
