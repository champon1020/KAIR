...
def main():
    ...
    logger.info(
        "model_name:{}, model sigma:{}, image sigma:{}".format(
    ...
    if "nf" not in model_name:  # noise-free SR
        degradation_vector = np.append(degradation_vector, noise_level_model / 255.0)
    ...
    for idx, img in enumerate(L_paths):
        ...
        util.imshow(
            ...
            title="LR image with noise level {}".format(noise_level_img),
    ...
    if need_H:
        ...
        logger.info(
            "Average PSNR/SSIM(RGB) - {} - x{} --PSNR: {:.2f} dB; SSIM: {:.4f}".format(
                result_name, sf, ave_psnr, ave_ssim
            )
        )
        if np.ndim(img_H) == 3:
            ...
            logger.info(
                "Average PSNR/SSIM( Y ) - {} - x{} - PSNR: {:.2f} dB; SSIM: {:.4f}".format(
                    result_name, sf, ave_psnr_y, ave_ssim_y
                )
            )
