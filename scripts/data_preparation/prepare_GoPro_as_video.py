...
def rearrange_dir_structure(dataset_path, traintest="train"):
    ...
    for path in file_list:
        ...
        shutil.move(
            ...
            os.path.join(f"{dataset_path}/{traintest}_GT", name),
        )
        shutil.move(
            os.path.join(path, "blur"),
            os.path.join(f"{dataset_path}/{traintest}_GT_blurred", name),
        )
...
def generate_meta_info_txt(data_path, meta_info_path):
    ...
    assert (
        total_frames == 2103
    ), f"GoPro training set should have 2103 images, but got {total_frames} images"
