...
def rearrange_dir_structure(dataset_path):
    ...
    for path in file_list:
        ...
        shutil.move(
            ...
            os.path.join(f"{dataset_path}/train_GT_with_val", name),
        )
        shutil.move(
            os.path.join(path, "input"),
            os.path.join(f"{dataset_path}/train_GT_blurred_with_val", name),
        )
        shutil.rmtree(path)
...
def generate_meta_info_txt(data_path, meta_info_path):
    ...
    assert (
        total_frames == 6708
    ), f"DVD training+Validation set should have 6708 images, but got {total_frames} images"
