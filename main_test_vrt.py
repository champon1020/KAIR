...
from data.dataset_video_test import (VFI_DAVIS, VFI_UCF101,
                                     SingleVideoRecurrentTestDataset, VFI_Vid4,
...
def main():
    ...
    parser.add_argument(
        "--task",
        type=str,
        default="001_VRT_videosr_bi_REDS_6frames",
        help="tasks: 001 to 008",
    )
    parser.add_argument(
        "--sigma",
        type=int,
    ...
    test_loader = DataLoader(
        dataset=test_set, num_workers=args.num_workers, batch_size=1, shuffle=False
    )
    ...
    for idx, batch in enumerate(test_loader):
        ...
        for i in range(output.shape[1]):
            ...
            if gt is not None:
                ...
                if img_gt.ndim == 3:  # RGB image
                    ...
                    test_results_folder["psnr_y"].append(
                        util.calculate_psnr(img, img_gt, border=0)
                    )
                    test_results_folder["ssim_y"].append(
                        util.calculate_ssim(img, img_gt, border=0)
                    )
                else:
        ...
        if gt is not None:
            ...
            psnr_y = sum(test_results_folder["psnr_y"]) / len(
                test_results_folder["psnr_y"]
            )
            ssim_y = sum(test_results_folder["ssim_y"]) / len(
                test_results_folder["ssim_y"]
            )
...
def prepare_model_dataset(args):
    ...
    if args.task == "001_VRT_videosr_bi_REDS_6frames":
        ...
        datasets = [
            "Vid4"
        ]  # 'Vimeo'. Vimeo dataset is too large. Please refer to #training to download it.
        args.scale = 4
        args.window_size = [8, 8, 8]
        args.nonblind_denoising = False
    ...
    if os.path.exists(model_path):
        ...
        url = "https://github.com/JingyunLiang/VRT/releases/download/v0.0/{}".format(
    ...
    if os.path.exists(f"{args.folder_lq}"):
        ...
        if "vimeo" in args.folder_lq.lower():
            print(
                f"Vimeo dataset is not at {args.folder_lq}! Please refer to #training of Readme.md to download it."
            )
        else:
