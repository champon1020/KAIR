...
def main():
    ...
    n_channels = (
        1 if "gray" in model_name else 3
    )  # 3 for color image, 1 for grayscale image
    ...
    )  # L_path and H_path, fixed, for Low-quality images
        ...
        for k_index in range(kernels.shape[1]):
            ...
            for img in L_paths:
                ...
                img_L = ndimage.filters.convolve(
                    img_H, kernel[..., np.newaxis], mode="wrap"
                )  # blur
                img_L = sr.downsample_np(
                    img_L, sf, center=False
                ...
                psnr = util.calculate_psnr(
                    img_E, img_H, border=sf**2
                )  # change with your own border
                test_results["psnr"].append(psnr)
                logger.info(
                    "{:->4d}--> {:>10s} -- x{:>2d} --k{:>2d} PSNR: {:.2f}dB".format(
                        idx, img_name + ext, sf, k_index, psnr
                    )
                )
