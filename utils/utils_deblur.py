...
import scipy
import torch
from numpy import (arange, array, exp, log, mgrid, min, mod, ones, pad, pi,
                   prod, round, sum, zeros)
...
def otf2psf(otf, outsize=None):
    ...
    if type(outsize) != type(None):
        ...
        outsize = np.pad(
            colvec_out, ((0, max(0, n - np.size(colvec_out))), (0, 0)), mode="constant"
        )
        insize = np.pad(
            colvec_in, ((0, max(0, n - np.size(colvec_in))), (0, 0)), mode="constant"
        )
...
def solve_min_laplacian(boundary_image):
    ...
    f_bp = np.zeros((H, W))
    f_bp[np.ix_(j, k)] = (
        -4 * boundary_image[np.ix_(j, k)]
        + boundary_image[np.ix_(j, k + 1)]
        + boundary_image[np.ix_(j, k - 1)]
        + boundary_image[np.ix_(j - 1, k)]
        + boundary_image[np.ix_(j + 1, k)]
    )
    ...
    denom = (2 * np.cos(np.pi * x / (W - 1)) - 2) + (
        2 * np.cos(np.pi * y / (H - 1)) - 2
    )
    ...
    if tt.shape[1] == 1:
        img_tt = np.transpose(
            fftpack.idst(np.transpose(tt) * 2, type=1) / (2 * (tt.shape[0] + 1))
        )
    else:
        img_tt = np.transpose(
            fftpack.idst(np.transpose(tt) * 2, type=1, axis=0) / (2 * (tt.shape[1] + 1))
        )
    del tt
...
def blurkernel_synthesis(h=37, w=None):
    ...
    if np.random.randint(0, 4) == 1:
        k = cv2.resize(
            k,
            (random.randint(x1, 5 * x1), random.randint(x2, 5 * x2)),
            interpolation=cv2.INTER_LINEAR,
        )
        y1, y2 = k.shape
        k = k[
            (y1 - x1) // 2 : (y1 - x1) // 2 + x1, (y2 - x2) // 2 : (y2 - x2) // 2 + x2
        ]
