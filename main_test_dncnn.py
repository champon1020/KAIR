...
def main():
    ...
    parser.add_argument(
        "--testset_name", type=str, default="set12", help="test set, bsd68 | set12"
    )
    parser.add_argument(
        "--noise_level_img", type=int, default=15, help="noise level: 15, 25, 50"
    )
    parser.add_argument(
        "--x8", type=bool, default=False, help="x8 to boost performance"
    )
    parser.add_argument("--show_img", type=bool, default=False, help="show the image")
    parser.add_argument(
        "--model_pool", type=str, default="model_zoo", help="path of model_zoo"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--need_degradation", type=bool, default=True, help="add noise or not"
    )
    parser.add_argument(
        "--task_current", type=str, default="dn", help="dn for denoising, fixed!"
    )
    ...
    border = (
        args.sf if args.task_current == "sr" else 0
    ...
    logger.info(
        "model_name:{}, image sigma:{}".format(args.model_name, args.noise_level_img)
    )
    logger.info(L_path)
    ...
    for idx, img in enumerate(L_paths):
        ...
        if args.need_degradation:  # degradation process
            ...
            img_L += np.random.normal(0, args.noise_level_img / 255.0, img_L.shape)

        util.imshow(
            util.single2uint(img_L),
            title="Noisy image with noise level {}".format(args.noise_level_img),
        ...
        if need_H:
            ...
            util.imshow(
                np.concatenate([img_E, img_H], axis=1), title="Recovered / Ground-truth"
            ) if args.show_img else None
