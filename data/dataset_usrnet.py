...
from utils import utils_deblur, utils_sisr
...
class DatasetUSRNet(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.sigma_max = (
            self.opt["sigma_max"] if self.opt["sigma_max"] is not None else 25
        )
        self.scales = opt["scales"] if opt["scales"] is not None else [1, 2, 3, 4]
        self.sf_validation = (
            opt["sf_validation"] if opt["sf_validation"] is not None else 3
        )
        # self.kernels = hdf5storage.loadmat(os.path.join('kernels', 'kernels_12.mat'))['kernels']
        self.kernels = loadmat(os.path.join("kernels", "kernels_12.mat"))[
            "kernels"
        ]  # for validation
    ...
    def __getitem__(self, index):
        ...
        if self.opt["phase"] == "train":
            ...
            if r_value > 3:
                ...
                k = utils_sisr.gen_kernel(
                    scale_factor=np.array([sf_k, sf_k])
                )  # Gaussian blur
            ...
            img_L = ndimage.filters.convolve(
                patch_H, np.expand_dims(k, axis=2), mode="wrap"
            )
            ...
            k /= np.sum(k)
            noise_level = 0.0 / 255.0  # validation noise level
            ...
            img_L = ndimage.filters.convolve(
                img_H, np.expand_dims(k, axis=2), mode="wrap"
            )  # blur
            img_L = img_L[
                0 :: self.sf_validation, 0 :: self.sf_validation, ...
            ]  # downsampling
            img_L = util.uint2single(img_L) + np.random.normal(
                0, noise_level, img_L.shape
            )
