...
class VideoRecurrentTrainDataset(data.Dataset):
            ...
            for line in fin:
                ...
                keys.extend(
                    [
                        ...
                        for i in range(
                            int(start_frame), int(start_frame) + int(frame_num)
                        )
                    ]
                )
    ...
    def __getitem__(self, index):
        ...
        endmost_start_frame_idx = (
            start_frames + total_num_frames - self.num_frame * interval
        )
        ...
        for neighbor in neighbor_list:
            if self.is_lmdb:
                ...
                img_lq_path = (
                    self.lq_root
                    / clip_name
                    / f"{neighbor:{self.filename_tmpl}}.{self.filename_ext}"
                )
                img_gt_path = (
                    self.gt_root
                    / clip_name
        ...
        img_results = utils_video.augment(
            img_lqs, self.opt["use_hflip"], self.opt["use_rot"]
        )
...
class VideoRecurrentTrainNonblindDenoisingDataset(VideoRecurrentTrainDataset):
    ...
    def __getitem__(self, index):
        ...
        # randomly crop
        img_gts, _ = utils_video.paired_random_crop(
            img_gts, img_gts, self.gt_size, 1, img_gt_path
        )

        # augmentation - flip, rotate
        img_gts = utils_video.augment(
            img_gts, self.opt["use_hflip"], self.opt["use_rot"]
        )
