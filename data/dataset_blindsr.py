...
class DatasetBlindSR(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.degradation_type = (
            opt["degradation_type"] if opt["degradation_type"] else "bsrgan"
        )
        self.lq_patchsize = self.opt["lq_patchsize"] if self.opt["lq_patchsize"] else 64
        self.patch_size = (
            self.opt["H_size"] if self.opt["H_size"] else self.lq_patchsize * self.sf
        )
    ...
    def __getitem__(self, index):
        ...
        if H < self.patch_size or W < self.patch_size:
            img_H = np.tile(
                np.random.randint(0, 256, size=[1, 1, self.n_channels], dtype=np.uint8),
