...
class DatasetFDnCNN(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.opt["phase"] == "train":
            ...
            noise_level = (
                torch.FloatTensor([np.random.uniform(self.sigma_min, self.sigma_max)])
                / 255.0
            )

            noise_level_map = (
                torch.ones((1, img_L.size(1), img_L.size(2))).mul_(noise_level).float()
            ...
            noise_level_map = (
                torch.ones((1, img_L.shape[0], img_L.shape[1]))
                .mul_(self.sigma_test / 255.0)
                .float()
            )  # torch.full((1, img_L.size(1), img_L.size(2)), noise_level)
