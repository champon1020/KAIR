...
from .op.deform_attn import DeformAttnPack, deform_attn
...
class GuidedDeformAttnPack(DeformAttnPack):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.conv_offset = nn.Sequential(
            nn.Conv3d(
                self.in_channels * (1 + self.clip_size) + self.clip_size * 2,
                64,
                kernel_size=(1, 1, 1),
                padding=(0, 0, 0),
            ),
    ...
    def forward(self, q, k, v, v_prop_warped, flows, return_updateflow):
        ...
        v = deform_attn(
            ...
            self.clip_size,
        ).view(b, t, self.proj_channels, h, w)
        v = self.proj(v)
        v = v + self.mlp(v)
        ...
        if return_updateflow:
            return (
                v,
                offset1.view(b, t, c // 2, 2, h, w).mean(2).flip(2),
                offset2.view(b, t, c // 2, 2, h, w).mean(2).flip(2),
            )
        else:
            return v
...
class WindowAttention(nn.Module):
    ...
    def attention(self, q, k, v, mask, x_shape):
        ...
        attn = F.softmax(
            attn, -1, dtype=q.dtype
        )  # Don't use attn.dtype after addition!
...
class STG(nn.Module):
    ...
    def __init__(
        ...
        self.window_size = window_size
        self.shift_size = (
            list(i // 2 for i in window_size) if shift_size is None else shift_size
        )
    ...
    def __init__(
        ...
        if self.upscale == 4:
            ...
            self.feat_extract = nn.Sequential(
                Rearrange("n d c h w -> n c d h w"),
                nn.Conv3d(
                    4 if self.nonblind_denoising else 3,
                    embed_dims[0],
                    (1, 3, 3),
                    (1, 2, 2),
                    (0, 1, 1),
                ...
                nn.LeakyReLU(negative_slope=0.1, inplace=True),
                nn.Conv3d(
                    embed_dims[0], embed_dims[0], (1, 3, 3), (1, 2, 2), (0, 1, 1)
                ),
                nn.LeakyReLU(negative_slope=0.1, inplace=True),
        ...
        self.conv_before_upsampler = nn.Sequential(
            nn.Conv3d(embed_dims[-1], 64, kernel_size=(1, 1, 1), padding=(0, 0, 0)),
            nn.LeakyReLU(negative_slope=0.1, inplace=True),
        )
        ...
        for i in range(0, len(clip_idx)):
            ...
            if i > 0:
                if "_1" in module_name:
                    ...
                    flow_n02 = flow_n12 + flow_warp(
                        flow_n01, flow_n12.permute(0, 2, 3, 1)
                    )
                    flow_n13 = flow_n23 + flow_warp(
                        flow_n12, flow_n23.permute(0, 2, 3, 1)
                    )
                    flow_n03 = flow_n23 + flow_warp(
                        flow_n02, flow_n23.permute(0, 2, 3, 1)
                    )
            ...
            if "backward" in module_name:
                feat = [
                    feats[k][idx_c].flip(1) for k in feats if k not in [module_name]
                ] + [feat_prop]
            else:
    ...
    def upsample(self, lqs, feats):
        ...
        if self.cpu_cache:
            ...
            for i in range(0, feats["shallow"].shape[1]):
                ...
                hr = self.conv_last(
                    self.upsampler(self.conv_before_upsampler(hr.transpose(1, 2)))
                ).transpose(1, 2)
    ...
    def forward(self, lqs):
        ...
        if self.upscale == 4:
            ...
            lqs_downsample = F.interpolate(
                lqs[:, :, :3, :, :].view(-1, 3, h, w), scale_factor=0.25, mode="bicubic"
            ).view(n, t, 3, h // 4, w // 4)
        ...
        if self.cpu_cache:
            ...
            for i in range(0, t // self.clip_size):
                feat = self.feat_extract(
                    lqs[:, i * self.clip_size : (i + 1) * self.clip_size, :, :, :]
                ).cpu()
            ...
            feats["shallow"] = list(
                torch.chunk(self.feat_extract(lqs), t // self.clip_size, dim=1)
            )
            flows_forward, flows_backward = self.compute_flow(lqs_downsample)
