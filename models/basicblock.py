...
class ESA(nn.Module):
    ...
    def forward(self, x):
        ...
        x2 = F.interpolate(
            self.conv5(x2), (x.size(2), x.size(3)), mode="bilinear", align_corners=False
        )
...
class CFRB(nn.Module):
    def __init__(
        ...
        self.conv2_r = conv(
            self.r_nc, self.r_nc, kernel_size, stride, padding, bias=bias, mode=mode[0]
        )
        self.conv3_d = conv(
            self.r_nc,
            self.d_nc,
        ...
        self.conv4_d = conv(
            self.r_nc, self.d_nc, kernel_size, stride, padding, bias=bias, mode=mode[0]
        )
        self.conv1x1 = conv(
            self.d_nc * 4,
            out_channels,
