...
def download_pretrained_model(model_dir="model_zoo", model_name="dncnn3.pth"):
    if os.path.exists(os.path.join(model_dir, model_name)):
        ...
        if "SwinIR" in model_name:
            ...
            url = (
                "https://github.com/JingyunLiang/VRT/releases/download/v0.0/{}".format(
                    model_name
                )
            )
        elif "_RVRT_" in model_name:
            url = (
                "https://github.com/JingyunLiang/RVRT/releases/download/v0.0/{}".format(
                    model_name
                )
            )
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_dir", type=str, default="model_zoo", help="path of model_zoo"
    )
        ...
        for method_model in args.models:
            if method_model in method_zoo:  # method, need for loop
                for model_name in method_model_zoo[method_model]:
                    if "SwinIR" in model_name:
                        download_pretrained_model(
                            os.path.join(args.model_dir, "swinir"), model_name
                        )
                    elif "_VRT_" in model_name:
                        download_pretrained_model(
                            os.path.join(args.model_dir, "vrt"), model_name
                        )
                    elif "_RVRT_" in model_name:
                        download_pretrained_model(
                            os.path.join(args.model_dir, "rvrt"), model_name
                        )
                    else:
                        download_pretrained_model(args.model_dir, model_name)
                ...
                if "SwinIR" in method_model:
                    download_pretrained_model(
                        os.path.join(args.model_dir, "swinir"), method_model
                    )
                elif "_VRT_" in method_model:
                    download_pretrained_model(
                        os.path.join(args.model_dir, "vrt"), method_model
                    )
                elif "_RVRT_" in method_model:
                    download_pretrained_model(
                        os.path.join(args.model_dir, "rvrt"), method_model
                    )
                else:
